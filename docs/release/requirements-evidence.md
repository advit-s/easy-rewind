...
| S1-06 | Purge DB/WAL/SHM/settings and real `.env` from worktree | Stage 1 | implemented | manifest-bound purge removed the four data/settings files; `.env` removal remains pending | live purge count `4`, post-purge absence `4/4`, backup rehash `4/4`; hygiene check pending | `docs/release/stage-1-verification.md` Purge evidence | Recover data only through a disposable verified quarantine copy; recreate non-secret configuration from `.env.example` and provision a replacement key through the protected channel—never imply that `.env` is in quarantine |
